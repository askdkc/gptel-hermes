...
```bash
# Use hermes_terminal to run the specific test
...
Use `hermes_terminal` to run tests at each step:

```python
# RED — verify failure
hermes_terminal(program="pytest", arguments=["tests/test_feature.py::test_name", "-v"])

# GREEN — verify pass
hermes_terminal(program="pytest", arguments=["tests/test_feature.py::test_name", "-v"])

# Full suite — verify no regressions
hermes_terminal(program="pytest", arguments=["tests/", "-q"])
```

### If another agent is unavailable

Keep the same RED-GREEN-REFACTOR sequence in the current agent. Run the focused
test, implementation, and full suite directly with `hermes_terminal`; do not
claim that work was delegated when it was not.
